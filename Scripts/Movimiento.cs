...
public class Movimiento : MonoBehaviour {
    ...
    public IEnumerator Muerte(){
       ...
       // rb.isKinematic = true;    //no le afecta la gravedad
        ...
        if (gameover==false) { // mientras haya vidas
            ...
            if (check.GetComponent<Checkpoint>().checkpoint == true) {
                ...
                transform.position = new Vector3(40.65f, -4.34191f, 0);
                animator.SetBool("muerte", false);
                enemigos.GetComponent<RestaurarEnemigos>().Reactivar();

            } else{   // llevar al punto de control si ha pasado

                transform.position = new Vector3(-9.9f, -4.34191f, 0);
                animator.SetBool("muerte", false);
                enemigos.GetComponent<RestaurarEnemigos>().Reactivar();// llama a la funcion de el script restaurearenemigo
                                                                       // de  la clasedel gameobject enemigo
